...
use crate::{
    ...
    image::GenericImage,
    mutex::{RwLockReadGuard, ShimRwLock},
    runtime::Runtime,
};
use core::{mem, ptr::NonNull};
use cty::{c_int, c_uint, c_ulong};
use euclid::Size2D;
use owning_ref::OwningRef;
use storagevec::StorageMap;
use x11nas::xlib::{self, Display, Pixmap, _XGC};
...
impl X11Pixmap {
    ...
    pub fn from_image(
        ...
        for x in 0..width {
            for y in 0..height {
                let (r, g, b, _) = img.color_at(x, y).unwrap().convert_elements::<u8>();
        ...
        // delete the image
        unsafe { xlib::XDestroyImage(image.as_ptr()) };

        // finish up, construct the structure
        Ok(Self {
            inner: pixmap,
            display: *dpy,
            graphics_context: monitor_gc,
...
impl PixmapStorage {
    ...
    pub fn new() -> Self {
        Self {
            pixmaps: ShimRwLock::new(StorageMap::new()),
        }
    }
    ...
    pub fn register_image(
        ...
        runtime: &Runtime,
        monitor: c_int,
    ) -> crate::Result<OwningRef<RwLockReadGuard<'_, StorageMap<usize, X11Pixmap, 12>>, X11Pixmap>>
    {
        let id = img.id();
        if !self.pixmaps.read().contains_key(&id) {
            let mut pixmaps = self.pixmaps.write();
            pixmaps.insert(id, X11Pixmap::from_image(img, runtime, monitor)?);
            mem::drop(pixmaps);
            Ok(OwningRef::new(self.pixmaps.read()).map(move |pm| pm.get(&id).unwrap()))
        } else {
            Ok(OwningRef::new(self.pixmaps.read()).map(move |pm| pm.get(&id).unwrap()))
        }
    }
